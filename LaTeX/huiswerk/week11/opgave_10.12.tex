...
\answer{
    ...
    Hiervoor moeten we een chikwadraattoets uitvoeren op basis van de gegeven verdeling van opleidingsniveaus onder de gehele populatie.
    Allereerst definiëren we de volgende nulhypothese $H_0$ en de alternatieve hypothese $H_1$:
    \begin{itemize}
        \item[$H_0$:] de gekozen steekproef is representatief voor de populatie, oftewel de steekproef bestaat uit aantallen personen per opleidingsniveau naar rato van hoe vaak deze voorkomt in de totale Nederlandse bevolking.
        \item[$H_1$:] de gekozen steekproef is NIET representatief voor de .
    \end{itemize}
    Verder is het significantieniveau $\alpha = 0.05$ en de steekproefdata (zie tabel) gegeven.
    Om te toetsen of de gegeven steekproefdata (observed) overeenkomen representatief zijn,
    moeten we allereerst de verwachte frequenties berekenen gebaseerd op de populatiepercentages.
    Dit kunnen we doen door de populatiepercentages om te rekenen naar verwachte frequenties gegeven dat de steekproef uit $480$ kiesgerechtigde Nederlanders bestaat:
    \begin{center}
        \renewcommand{\arraystretch}{1.25}
        \begin{tabular}{ccc}
            \toprule
    ...
    \end{align*}
    Deze grenswaarde berekenen we met de \enquote{numerieke solver} door het volgende in te voeren:
    \begin{align*}
    ...
    \vspace{1em}
    {
        ...
        Dit kan erop duiden dat mensen met een hoger opleidingsniveau eerder geneigd zijn om deel te nemen aan een dergelijk opinieonderzoek.
    }
}
