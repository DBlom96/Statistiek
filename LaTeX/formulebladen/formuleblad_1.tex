...
Gegeven is een steekproef van $n$ observaties $x_1, x_2, \ldots, x_n$ uit een populatie met $N$ elementen (waarbij $N >> n$).
...
\begin{align}
    s^2 &= \frac{\sum_{i} (x_i-\overline{x})^2}{n} \hfill \tag{optie 1} \\%= \frac{(x_1-\overline{x})^2 + (x_2-\overline{x})^2 + \ldots + (x_n - \overline{x})^2}{n - 1}
    s^2 &= \frac{\sum_{i} x_i^2 - n \cdot \overline{x}^2}{n} \hfill \tag{optie 2}
\end{align}
\textbf{Merk op:} als de steekproef gebruikt wordt om de populatievariantie te \underline{schatten}, delen door $n-1$.
\vspace{.5cm}
...
\begin{align*}
    ...
    P(A \text{ gegeven } B)                &= \frac{P(A \text{ en } B)}{P(B)} \tag{conditionele kansen}\\
\end{align*}
...
\begin{table}[H]
    ...
    \begin{tabular}{m{3.8cm}|m{6cm}|m{5.7cm}}
        ...
        \midrule
            \textbf{Verwachtingswaarde:}                    & $E[X] = \sum_{k} k \cdot P(X=k)$ & $E[X] = \int_{x} x \cdot f(x)\ dx$\\
        \midrule
            \textbf{Variantie:}                             & $\text{Var}(X) = \sum_{k} (k - E[X])^2 \cdot P(X=k)$   & $\text{Var}(X) = \int_{x} (x - E[X])^2 \cdot f(x)\ dx$ \\    
        \midrule
