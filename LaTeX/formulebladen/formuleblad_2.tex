\subsection*{\SI{100\qtyproduct(1-\alpha)}{\percent}-betrouwbaarheidsinterval voor het gemiddelde $\mu$ (als $\sigma$ bekend is)}
    \begin{align*}
        z_{\alpha/2} = \invnorm(\opp=1-\alpha/2, \mu=0, \sigma=1)
    \end{align*}
        \[
            z_{\alpha/2} = \invnorm(\opp=1-\alpha/2, \mu=0, \sigma=1)
        \]
        \[
            [\overline{x} - z_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}}, \quad  \overline{x} + z_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}}]
        \]

        \item \textbf{Minimale steekproefomvang voor \boldmath$\SI{100(1-\alpha)}{\percent}$-BI als \boldmath$\mu$ maximaal \boldmath$\pm\ a$ mag afwijken:}
        \[
...
\subsection*{Betrouwbaarheidsintervallen voor het gemiddelde $\mu$ ($\sigma$ onbekend)}
    \begin{itemize}
        \item \textbf{\boldmath$\SI{100(1-\alpha)}{\percent}$-betrouwbaarheidsinterval (BI) voor \boldmath$\mu$:}
            \[
                t = \invt(\opp=1-\alpha/2, \text{df}=n-1)
            \]
            \[
                [\overline{x} - t \cdot \frac{s}{\sqrt{n}}, \quad \overline{x} + t \cdot \frac{s}{\sqrt{n}}]
            \]

        \item \textbf{Minimale steekproefomvang voor $\SI{100(1-\alpha)}{\percent}$-BI als $\mu$ maximaal \boldmath$\pm\ a$ mag afwijken:}
            \[
                \text{GR tabel (voor verschillende $n$):} \quad \frac{s}{\sqrt{n}} \cdot \text{InvT}(\text{area}=1-\alpha/2, \text{df}=n-1) \le a
            \]
            
            {\itshape NB: zodra $n \ge 30$, vallen de normale en de $t$-verdeling nagenoeg samen. Je mag dan rekenen met de schatting $s$ (steekproefstandaardafwijking) in plaats van de daadwerkelijke (onbekende) $\sigma$.}
    \end{itemize}
\subsection*{Betrouwbaarheidsintervallen voor de binomiale succeskans $p$}
    ...
    \begin{enumerate}
        \item Bereken (met \enquote{numeric solver}) de succeskans $p_1$ zodat geldt $P(X \le k) = \binomcdf(n, p_1, k) = \alpha/2$.
        \item Bereken (met \enquote{numeric solver}) de succeskans $p_2$ zodat geldt $P(X \ge k) = 1 - \binomcdf(n, p_2, k-1) = \alpha/2$.
        \item De berekende waarden voor $p_1$ en $p_2$ zijn de grenzen van het Clopper-Pearson interval.
    \end{enumerate}

\subsection*{Stappenplan hypothesetoetsen}
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item Kritiek gebied: verwerp $H_0$ als de toetsingsgrootheid een waarde aanneemt in het kritiek gebied, anders accepteer $H_0$.
                \item $p$-waarde: verwerp $H_0$ als $p \le \alpha$ (links-/rechtszijdig) of $p \le \frac{\alpha}{2}$ (tweezijdig), anders accepteer $H_0$.
            \end{itemize}
        \end{itemize}
    ...
    \textbf{Kritieke gebieden uitrekenen}
    \begin{table}[H]
        \centering
    ...
    \newpage
    \textbf{Soorten hypothesetoetsen}
    \begin{table}[H]
        \centering
        \renewcommand{\arraystretch}{1.5}
        \begin{tabular}{c|c|c}
            ...
            \midrule
                ...
                $t$-toets (geval $\sigma_A^2 = \sigma_B^2$) & $T = \frac{(\overline{X_A} - \overline{X_B}) - (\mu_A - \mu_B)}{\sqrt{\frac{S_P^2}{n} + \frac{S_P^2}{m}}}$ & $t(\text{df}=n+m-2)$ \\
...
\textbf{\boldmath$\SI{100\cdot(1-\alpha)}{\percent}$-betrouwbaarheidsinterval voor de gemiddelde $Y$ bij een gegeven $X = x_0$:}
\[
...
\textbf{\boldmath$\SI{100\cdot(1-\alpha)}{\percent}$-betrouwbaarheidsinterval voor $Y$ bij een gegeven $X = x_0$:}
\[
    t = \invt(\text{opp} = 1 - \alpha/2, \text{df} = n - 2)
\]
\[
