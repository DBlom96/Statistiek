\begin{question}{28}{
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    De Luchtmacht wil graag weten of de gemiddelde reactietijd significant verschilt tussen de twee typen boordcomputers.
    Je mag aannemen dat de reactietijden bij gebruik van beide typen boordcomputers normaal verdeeld zijn met onbekende verwachtingswaarden en standaardafwijkingen.
}   
    ...
    \solution{
        ...
        Dat betekent dat we met de \enquote{pooled variance} mogen werken als schatting voor de gemeenschappelijke onbekende $\sigma$.
        ...
        \begin{align*}
            g_1 &= \invt(\text{opp} = \alpha/2, \text{df}=n+m-2) = \invt(\text{opp}=0.025, \text{df}=19) \approx -2.0930 \\ \rubric{1}
            g_2 &= \invt(\text{opp} = 1-\alpha/2, \text{df}=n+m-2) = \invt(\text{opp}=0.975, \text{df}=19) \approx 2.0930
        \end{align*}
