\begin{enquestion}{20}{
    ...
    \begin{align*}
        f_{X,Y}(x,y) =  \begin{cases}
                            C \cdot (3x+2y), & \text{if } 0 \le x \le 2 \text{ and } 0 \le y \le 3 \\
                            0, & \text{otherwise,}
                        \end{cases}
    \end{align*}
    ...
    \solution{
        ...
        \begin{enumerate}
            \item First condition: $f_{X,Y}(x,y)\ge 0$ for all possible values of $x$ and $y$, and
            \item Second condition: $\int_{-\infty}^{\infty} \int_{-\infty}^{\infty} f_{X,Y}(x,y)\dx\dy=1$. \rubric{1}
        \end{enumerate}
        The first condition holds if and only if $C(3x+2y)$ for $0\le x\le 2$ and $0 \le y \le 3$, so we need $C \ge 0$. \rubric{1}
